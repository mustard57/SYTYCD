...
*/
  ...
  jointPredicates["nic_client"] = {name: "nic_client", title: "NIC Client", iri: "http://www.ourcompany.com/ontology/NewInsuranceCo/CLIENT", shortiri: "nic:client"};
  jointPredicates["nkb_customer"] = {name: "nkb_customer", title: "NKB Customer", iri: "http://www.ourcompany.com/ontology/NationalKensingtonBank/CUSTOMER", shortiri: "nkb:customer"};
  jointPredicates["jointname"] = {name: "jointname", title: "Full Name", iri: "http://www.ourcompany.com/ontology/JointCustomer/name", shortiri: "joint:name"};
  ...
  // Add our graph explorer
    ...
    var resUpdater = {
      updateSubjectResults: function (results) {
        var entities = results.head.vars;
        
        // get first result and draw it in our graph explorer
        ...
        if (bindings.length > 0) {
          ...
          for (var et = 0, maxent = entities.length, entityValue, type;null == uri && et < maxent;et++) {
            entityValue = binding[entities[et]];
            type = binding[entities[et]].type;
            if ("uri" == type) {
              uri = entityValue.value;
            }
          }
    ...
    semctx.register(resUpdater);
    console.log("Done reg updater");
  
  


var sparql = "PREFIX foaf: <http://xmlns.com/foaf/0.1/> PREFIX rdfs: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX so: <http://www.schema.org/> SELECT distinct ?jointcustomer WHERE { ?jointcustomer rdfs:type <http://www.ourcompany.com/ontology/JointCustomer> . ?jointcustomer <http://www.ourcompany.com/ontology/NationalKensingtonBank/CUSTOMER> ?nkbcustomer1 . ?nkbcustomer1 rdfs:type <http://marklogic.com/rdb2rdf/NationalKensingtonBank/CUSTOMER> . ?nkbcustomer1 <http://marklogic.com/rdb2rdf/NationalKensingtonBank/CUSTOMER#ref-ACCOUNT_ID> ?nkbaccount2 . ?nkbaccount2 rdfs:type <http://marklogic.com/rdb2rdf/NationalKensingtonBank/ACCOUNT> . ?nkbaccount2 <http://marklogic.com/rdb2rdf/NationalKensingtonBank/ACCOUNT#BALANCE> ?a1691 . FILTER (?a1691 <= \"100\"^^<xs:double> ) . ?jointcustomer <http://www.ourcompany.com/ontology/NewInsuranceCo/CLIENT> ?nicclient3 . ?nicclient3 rdfs:type <http://marklogic.com/rdb2rdf/NewInsuranceCo/CLIENT> . ?nicclient3 <http://marklogic.com/semantics/ontology/mentioned_in> ?document4 . ?document4 rdfs:type <http://marklogic.com/semantics/ontology/Document> . } ";

    semctx.subjectQuery(sparql,0,10);
