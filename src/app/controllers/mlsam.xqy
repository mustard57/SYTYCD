...
{ (: SELECT ke.referenced_table_name parent, ke.table_name child, ke.constraint_name :)
  ...
  let $sqlQuery := "SELECT ke.* FROM information_schema.KEY_COLUMN_USAGE ke WHERE ke.TABLE_SCHEMA=""test2"" AND ke.referenced_table_name IS NULL AND ke.table_name=""player"" "
  return
  ch:add-value("mlsamResponse", sql:execute($sqlQuery, $config:MLSAM-URL, ())),
  ch:use-view((), "xml"),
  ch:use-layout(("mlsam"), "html")
